...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  const upgradeToPremium = async () => {
    try {
      ...
      // Trouver le plan Premium
      const premiumPlan = plans.find((p: any) => p.name.toLowerCase() === 'premium');
      if (!premiumPlan) {
        throw new Error('Plan Premium non trouvé');
      }

      // Utiliser le prix annuel par défaut
      const priceId = premiumPlan.stripePriceIdYearly;
      if (!priceId) {
        throw new Error('Prix Stripe non configuré pour Premium');
      }
  ...
  const upgradeToPro = async () => {
    try {
      ...
      if (!proPlan) {
        throw new Error('Plan Pro non trouvé');
      }

      // Utiliser le prix annuel par défaut
      const priceId = proPlan.stripePriceIdYearly;
      if (!priceId) {
        throw new Error('Prix Stripe non configuré pour Pro');
      }
