...
export function useUnlockedUnfollowers(accountId?: number | null) {
  return useQuery({
    queryKey: ['unlocked-unfollowers', accountId ?? null],
    queryFn: async () => {
      const url = accountId ? `/api/unfollowers/unlocked/list?accountId=${accountId}` : '/api/unfollowers/unlocked/list';
