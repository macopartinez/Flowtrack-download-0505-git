...
export class DataValidator {
     ...
     */
    static isValidMessageText(text) {
     ...
     */
    static isValidDMMessage(message) {
        if (!message || typeof message !== 'object') {
            return false;
        }
        return (this.isValidUsername(message.conversationWith) &&
     ...
     */
    static isValidContactScore(score) {
        if (!score || typeof score !== 'object') {
            return false;
        }
        return (this.isValidUsername(score.username) &&
...
export class DataSanitizer {
     ...
     */
    static sanitizeDMMessage(message) {
        return {
            ...
            mediaUrls: Array.isArray(message.mediaUrls)
                ? message.mediaUrls.map((url) => this.sanitizeUrl(url)).filter(Boolean)
                : [],
     ...
     */
    static sanitizeContactScore(score) {
...
export class RateLimiter {
     ...
     */
    static isAllowed(key, maxRequests, windowMs) {
        const now = Date.now();
...
export class CSPHelper {
     ...
     */
    static isAllowedScript(scriptUrl) {
