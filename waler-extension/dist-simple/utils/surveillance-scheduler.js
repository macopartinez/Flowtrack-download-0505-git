 ...
 */
export var SurveillanceCategory;
(function (SurveillanceCategory) {
    SurveillanceCategory["CLIENT"] = "client";
    SurveillanceCategory["PROSPECT"] = "prospect";
    SurveillanceCategory["NETWORK"] = "network";
    SurveillanceCategory["LEAD"] = "lead";
})(SurveillanceCategory || (SurveillanceCategory = {}));
export class SurveillanceScheduler {
     ...
     */
    static startSurveillance(username, category, onCheck) {
     ...
     */
    static stopSurveillance(username) {
     ...
     */
    static async performCheck(username, category, onCheck) {
     ...
     */
    static updateCategory(username, newCategory, onCheck) {
     ...
     */
    static getNextCheck(username, category) {
        const lastCheckTime = this.lastCheck.get(username);
        if (!lastCheckTime) {
     ...
     */
    static getContactsToCheck() {
        const contacts = [];
        const now = Date.now();
        for (const [key, _] of this.schedules.entries()) {
            const [username, category] = key.split('_');
...
SurveillanceScheduler.schedules = new Map();
SurveillanceScheduler.lastCheck = new Map();
SurveillanceScheduler.activeChecks = new Set();
/**
...
export class SurveillancePriorityManager {
     ...
     */
    static calculateDynamicPriority(category, factors) {
     ...
     */
    static recommendFrequencyAdjustment(category, activityScore) {
        const config = SurveillanceScheduler.getConfig(category);
...
export class SurveillanceClassificationBridge {
     ...
     */
    static async syncWithClassification(userId, onCheck) {
        try {
            ...
            for (const score of scores) {
                ...
                SurveillanceScheduler.startSurveillance(score.contact_username, category, onCheck);
            }
     ...
     */
    static mapCategoryToSurveillance(classificationCategory) {
        const mapping = {
     ...
     */
    static handleCategoryChange(username, oldCategory, newCategory, onCheck) {
