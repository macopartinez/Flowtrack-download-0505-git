...
export class ScoringEngine {
    ...
    calculateSeniorityScore(contact) {
        const now = Date.now();
        const daysSinceFirst = Math.floor((now - contact.firstInteractionDate) / (1000 * 60 * 60 * 24));
        ...
        const daysSinceLast = Math.floor((now - contact.lastInteractionDate) / (1000 * 60 * 60 * 24));
    ...
    determineSuggestedCategory(score, dmCategory) {
        ...
        if (dmCategory) {
            if (dmCategory === 'client' && score >= 80)
                return 'client';
            if (dmCategory === 'prospect' && score >= 50)
                return 'prospect';
        }
    ...
    detectTransition(contact, currentScore) {
        ...
        const suggestedCategory = this.determineSuggestedCategory(currentScore.total, contact.conversation
        ...
        const { reason, evidence, confidence } = this.analyzeTransitionReason(contact, currentCategory, suggestedCategory, currentScore);
        return {
