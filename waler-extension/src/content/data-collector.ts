...
export class DataCollector {
  ...
  async processUserInfo(data: any) {
    try {
      ...
      // GARDE PAR ID (la plus fiable) : ne traiter QUE le profil dont l'id == le
      // compte CONNECTÉ (cookie ds_user_id). Indépendant du pseudo, donc immunisé
      // au cas « on consulte le profil d'une People » : son user info ne doit
      // jamais être prise pour la nôtre (sinon faux compteurs + boucle de refresh).
      const loggedInId = (document.cookie.match(/ds_user_id=(\d+)/) || [])[1];
      const profileId = user.pk != null ? String(user.pk) : user.id != null ? String(user.id) : null;
