...
Si votre serveur FlowTrack tourne sur un autre port, modifiez `API_URL` dans :
- `src/background/sync-manager.ts`

### 5. Authentification

1. Connectez-vous à FlowTrack (`http://localhost:5000`)
...
- Vérifiez que le serveur FlowTrack tourne (`http://localhost:5000`)
...
- Vérifiez le dashboard FlowTrack
